<?php
//Register meta boxes
if(!function_exists('edgt_lms_quiz_meta_box_functions')) {
    function edgt_lms_quiz_meta_box_functions($post_types) {
        $post_types[] = 'quiz';

        return $post_types;
    }

    add_filter('educator_edge_meta_box_post_types_save', 'edgt_lms_quiz_meta_box_functions');
    add_filter('educator_edge_meta_box_post_types_remove', 'edgt_lms_quiz_meta_box_functions');
}

//Register meta boxes scope
if(!function_exists('edgt_lms_quiz_scope_meta_box_functions')) {
    function edgt_lms_quiz_scope_meta_box_functions($post_types) {
        $post_types[] = 'quiz';

        return $post_types;
    }

    add_filter('educator_edge_set_scope_for_meta_boxes', 'edgt_lms_quiz_scope_meta_box_functions');
}

//Register quiz post type
if(!function_exists('edgt_lms_register_quiz_cpt')) {
    function edgt_lms_register_quiz_cpt($cpt_class_name) {
        $cpt_class = array(
            'EdgefLMS\CPT\Quiz\QuizRegister'
        );
    ...
    add_filter('edgt_lms_filter_register_custom_post_types', 'edgt_lms_register_quiz_cpt');
}

//Quiz single functions
if(!function_exists('edgt_lms_get_single_quiz')) {
    function edgt_lms_get_single_quiz() {
        edgt_lms_get_cpt_single_module_template_part('templates/single/holder', 'quiz', '', array());
    }
}

//Getter and setter for user quiz values
if(!function_exists('edgt_lms_set_user_quiz_values')) {
    function edgt_lms_set_user_quiz_values($course_id = 0, $quiz_id = 0, $params = array(), $field = '') {
...
if(!function_exists('edgt_lms_get_user_quiz_values')) {
    function edgt_lms_get_user_quiz_values($course_id = 0, $quiz_id = 0, $field = '') {
...
if(!function_exists('edgt_lms_get_quiz_status_meta_params')) {
    function edgt_lms_get_quiz_status_meta_params($quiz_id = 0, $params = array()) {

        extract($params);

        $passing_percentage = get_post_meta($quiz_id, 'edgt_quiz_passing_percentage_meta', true);
...
if(!function_exists('edgt_lms_get_quiz_results_meta_params')) {
    function edgt_lms_get_quiz_results_meta_params($retake_id = 0, $params = array()) {
...
if(!function_exists('edgt_lms_get_quiz_temp_meta_params')) {
    function edgt_lms_get_quiz_temp_meta_params($quiz_id = 0, $params = array()) {
...
//Action functions
if(!function_exists('edgt_lms_start_quiz')) {
    function edgt_lms_start_quiz()
    {
        if(empty($_POST) || !isset($_POST)) {
            ...
            if($quiz_temp == '' || empty($quiz_temp)) {
                ...
                $params_results = edgt_lms_get_quiz_results_meta_params($retake);
                edgt_lms_set_user_quiz_values($course_id, $quiz_id, $params_results, 'edgt_user_quiz_results');

                //Update temporary field
                $params_results = edgt_lms_get_quiz_temp_meta_params($quiz_id);
                edgt_lms_set_user_quiz_values($course_id, $quiz_id, $params_results, 'edgt_user_quiz_temp');
            }
    ...
    add_action('wp_ajax_edgt_lms_start_quiz', 'edgt_lms_start_quiz');
}

if(!function_exists('edgt_lms_load_first_question')) {
    function edgt_lms_load_first_question()
    {
        if(empty($_POST) || !isset($_POST)) {
            edgt_lms_ajax_status('error', esc_html__('All fields are empty', 'edge-lms'));
        } else {
            ...
            $time_remaining_parameter = get_post_meta($quiz_id, 'edgt_quiz_duration_parameter_meta', true);
            $time_remaining = get_post_meta($quiz_id, 'edgt_quiz_duration_meta', true);
            ...
            $html = edgt_lms_cpt_single_module_template_part('templates/single/holder', 'quiz', 'active', $params);
            $json_data['html'] = $html;
    ...
    add_action('wp_ajax_edgt_lms_load_first_question', 'edgt_lms_load_first_question');
}

if(!function_exists('edgt_lms_finish_quiz')) {
    function edgt_lms_finish_quiz() {
        if(empty($_POST) || !isset($_POST)) {
            edgt_lms_ajax_status('error', esc_html__('All fields are empty', 'edge-lms'));
        } else {
            $data = $_POST;
            ...
            parse_str($data_string, $data_array);
            $quiz_id = $data_array['edgt_lms_quiz_id'];
            $item_id = $data_array['edgt_lms_quiz_id'];
            ...
            $time_total_formatted = edgt_lms_get_quiz_time_formated($quiz_id, $time_remaining);
            $params_status['time'] =  $time_total_formatted;

            $result = edgt_lms_calculate_result($course_id, $quiz_id);
            $params_status = array_merge($params_status, $result);

            edgt_lms_set_user_quiz_values($course_id, $quiz_id, $params_status, 'edgt_user_quiz_status');
            ...
            $params['post_message'] = get_post_meta($quiz_id, 'edgt_quiz_post_message_meta', true);
            $html  = edgt_lms_cpt_single_module_template_part('templates/single/holder', 'quiz', '', $params);
            $json_data['html'] = $html;
    ...
    add_action('wp_ajax_edgt_lms_finish_quiz', 'edgt_lms_finish_quiz');
}

if(!function_exists('edgt_lms_get_quiz_time_formated')) {
    function edgt_lms_get_quiz_time_formated($quiz_id = 0, $time_remaining = 0) {
        //get inital value for quiz duration
        $time_remaining_parameter = get_post_meta($quiz_id, 'edgt_quiz_duration_parameter_meta', true);
        $time_remaining_base = get_post_meta($quiz_id, 'edgt_quiz_duration_meta', true);
        switch($time_remaining_parameter) {
            case 'm':
...
if(!function_exists('edgt_lms_calculate_result')) {
    function edgt_lms_calculate_result($course_id = 0, $quiz_id = 0) {
        ...
        $questions = get_post_meta($quiz_id, 'edgt_quiz_question_meta', true);
        $required_p = get_post_meta($quiz_id, 'edgt_quiz_passing_percentage_meta', true);
        ...
        foreach($questions as $question) {
            ...
            if(isset($result_params)) {
                ...
                if(!isset($answer_value) || $answer_value == '') {
                    ...
                    $result = edgt_lms_validate_question($question, $answer_value, true);
                    $points += $result;
                    if ($result == 0) {
                        $q_wrong++;
                    } else {
        ...
        $points_total = 0;
        foreach($questions as $question) {
            $question_value = get_post_meta($question, 'edgt_question_mark_meta', true);
...
//Template functions
if(!function_exists('edgt_lms_template_start_quiz_button')) {
    function edgt_lms_template_start_quiz_button($params = array()) {

        $quiz_id = $params['item_id'];
        $course_id = $params['course_id'];
        $quiz_results = edgt_lms_get_user_quiz_values($course_id, $quiz_id, 'edgt_user_quiz_results');
        $disabled = '';

        if(!empty($quiz_results)) {
            $retakes_taken = count($quiz_results);
            $retakes_allowed = get_post_meta(get_the_ID(), 'edgt_quiz_number_retakes_meta', true);
            if($retakes_allowed != '' && $retakes_taken - 1 < $retakes_allowed) {
                $retakes_remain = $retakes_allowed - $retakes_taken + 1;
                $button_text = esc_html__('Start quiz', 'edge-lms') . ' +' . esc_attr($retakes_remain);
                $custom_attrs = array();
            }
            else {
                $retakes_taken = -1;
                $button_text = esc_html__('No attempts allowed', 'edge-lms');
                $custom_attrs = array(
                    'disabled' => 'disabled'
                );
                $disabled = 'disabled';
            ...
            $retakes_taken = 0;
            $button_text = esc_html__('Start quiz', 'edge-lms');
            $custom_attrs = array();
        }
        ...
        if($quiz_temp != '' && !empty($quiz_temp)) {
            ...
            $button_text = esc_html__('Resume quiz', 'edge-lms');
            $custom_attrs = array();
        }
        ...
        return edgt_lms_get_cpt_single_module_template_part('templates/single/parts/actions-start', 'quiz', '', $start_form_params);

    }
}

if(!function_exists('edgt_lms_template_quiz_info_top')) {
    function edgt_lms_template_quiz_info_top ($params = array()) {

        $questions = get_post_meta(get_the_ID(), 'edgt_quiz_question_meta', true);
        $questions_number = !empty($questions) ? count($questions) : 0;
        $questions_label = $questions_number == 1 ? esc_html__('Question', 'edge-lms') : esc_html__('Questions', 'edge-lms');
        $quiz_duration_value = get_post_meta(get_the_ID(), 'edgt_quiz_duration_meta', true);
        $quiz_duration_parameter = get_post_meta(get_the_ID(), 'edgt_quiz_duration_parameter_meta', true);
        ...
        $quiz_temp = edgt_lms_get_user_quiz_values($params['course_id'], $params['item_id'], 'edgt_user_quiz_temp');
        if($quiz_temp != '' && !empty($quiz_temp)) {
            if($quiz_temp['time_remaining'] != 0) {
        ...
        $info_top_params = array(
            ...
            'quiz_duration_parameter' => esc_html__('(hh:mm:ss)', 'edge-lms')
        );

        return edgt_lms_get_cpt_single_module_template_part('templates/single/parts/info-top', 'quiz', '', $info_top_params);
    }
}

if(!function_exists('edgt_lms_template_quiz_status')) {
    function edgt_lms_template_quiz_status ($params = array()) {

        $quiz_id = $params['item_id'];
        $course_id = $params['course_id'];

        $quiz_status = edgt_lms_get_user_quiz_values($course_id, $quiz_id, 'edgt_user_quiz_status');
        ...
        return edgt_lms_get_cpt_single_module_template_part('templates/single/parts/status', 'quiz', '', $status_params);
    }
}

if(!function_exists('edgt_lms_template_quiz_results')) {
    function edgt_lms_template_quiz_results ($params = array()) {
        ...
        return edgt_lms_get_cpt_single_module_template_part('templates/single/parts/results', 'quiz', '', $results_params);
    }
}