...
class CourseTable implements Lib\ShortcodeInterface {
    ...
    public function __construct() {
        $this->base = 'edgt_course_table';

        add_action('vc_before_init', array($this, 'vcMap'));

	    //Course category filter
	    add_filter( 'vc_autocomplete_edgt_course_table_category_callback', array( &$this, 'courseTableCategoryAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Course category render
	    add_filter( 'vc_autocomplete_edgt_course_table_category_render', array( &$this, 'courseTableCategoryAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Course selected projects filter
	    add_filter( 'vc_autocomplete_edgt_course_table_selected_courses_callback', array( &$this, 'courseTableIdAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Course selected projects render
	    add_filter( 'vc_autocomplete_edgt_course_table_selected_courses_render', array( &$this, 'courseTableIdAutocompleteRender', ), 10, 1 ); // Render exact portfolio. Must return an array (label,value)

	    //Course tag filter
	    add_filter( 'vc_autocomplete_edgt_course_table_tag_callback', array( &$this, 'courseTableTagAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array

	    //Course tag render
	    add_filter( 'vc_autocomplete_edgt_course_table_tag_render', array( &$this, 'courseTableTagAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array
    }
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map( array(
				    'name'                      => esc_html__( 'Edge Course Table', 'edge-lms' ),
				    'base'                      => $this->getBase(),
    ...
    public function render($atts, $content = null) {
	    ...
	    $html = edgt_lms_get_cpt_shortcode_module_template_part( 'course', 'course-table', '', $params, $additional_params );
