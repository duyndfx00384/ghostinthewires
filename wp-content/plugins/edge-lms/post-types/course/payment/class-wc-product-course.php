...
class WC_Product_Course extends WC_Abstract_Legacy_Product {
    ...
    protected $data = array(
        ...
        'backorders'         => 'no',
        'sold_individually'  => true,  /* BY EDGE */
        'weight'             => '',
        'length'             => '',
        'width'              => '',
        'height'             => '',
    ...
    public function get_price( $context = 'view' ) {
        return edgt_lms_calculate_course_price($this->get_id());
    }
    ...
    public function get_dimensions( $formatted = true ) {
        if ( $formatted ) {
            ...
            return apply_filters( 'edgt_lms_stock_photography_product_dimensions', wc_format_dimensions( $this->get_dimensions( false ) ), $this );
        }
        return array(
    ...
    */
    ...
    public function set_catalog_visibility( $visibility ) {
        ...
        if ( ! in_array( $visibility, $options, true ) ) {
            $this->error( 'product_invalid_catalog_visibility', __( 'Invalid catalog visibility option.', 'edge-lms' ) );
        }
    ...
    public function set_sku( $sku ) {
        ...
        if ( $this->get_object_read() && ! empty( $sku ) && ! wc_product_has_unique_sku( $this->get_id(), $sku ) ) {
            ...
            $this->error( 'product_invalid_sku', __( 'Invalid or duplicated SKU.', 'edge-lms' ), 400, array( 'resource_id' => $sku_found ) );
        }
    ...
    public function set_price( $price ) {
        $this->set_prop( 'price', edgt_lms_calculate_course_price($this->get_id()) );
    }
    ...
    public function set_tax_status( $status ) {
        ...
        if ( ! in_array( $status, $options, true ) ) {
            $this->error( 'product_invalid_tax_status', __( 'Invalid product tax status.', 'edge-lms' ) );
        }
    ...
    public function set_downloads( $downloads_array ) {
        ...
        foreach ( $downloads_array as $download ) {
            if ( is_a( $download, 'WC_Product_Download' ) ) {
                ...
                $file_hash                 = apply_filters( 'edgt_lms_course_downloadable_file_hash', md5( $download['file'] ), $this->get_id(), $download['name'], $download['file'], $download['previous_hash'] );
            ...
            if ( ! $download_object->is_allowed_filetype() ) {
                $errors[] = sprintf( __( 'The downloadable file %1$s cannot be used as it does not have an allowed file type. Allowed types include: %2$s', 'edge-lms' ), '<code>' . basename( $download_object->get_file() ) . '</code>', '<code>' . implode( ', ', array_keys( $download_object->get_allowed_mime_types() ) ) . '</code>' );
                continue;
            }

            // Validate the file exists.
            if ( ! $download_object->file_exists() ) {
                $errors[] = sprintf( __( 'The downloadable file %s cannot be used as it does not exist on the server.', 'edge-lms' ), '<code>' . $download_object->get_file() . '</code>' );
                continue;
            }
	...
	*/
    ...
    public function validate_props() {
        ...
        } elseif ( $this->get_stock_quantity() <= get_option( 'edgt_lms_course_notify_no_stock_amount' ) && 'no' === $this->get_backorders() ) {
            $this->set_stock_status( 'outofstock' );

            // If the stock level is changing and we do now have enough, force in stock status.
        } elseif ( $this->get_stock_quantity() > get_option( 'edgt_lms_course_notify_no_stock_amount' ) && array_key_exists( 'stock_quantity', $this->get_changes() ) ) {
    ...
    public function save() {
        ...
        if ( $this->data_store ) {
            ...
            do_action( 'edgt_lms_course_before_' . $this->object_type . '_object_save', $this, $this->data_store );
            ...
            if ( $this->get_parent_id() ) {
                wp_schedule_single_event( time(), 'edgt_lms_coursedeferred_product_sync', array( 'product_id' => $this->get_parent_id() ) );
            }
            return $this->get_id();
        }
    }
    ...
    */
    ...
    public function supports( $feature ) {
        return apply_filters( 'edgt_lms_course_product_supports', in_array( $feature, $this->supports ) ? true : false, $feature, $this );
    }
    ...
    public function is_downloadable() {
        return apply_filters( 'edgt_lms_course_is_downloadable', true === $this->get_downloadable(), $this );
    }
    ...
    public function is_virtual() {
        return apply_filters( 'edgt_lms_course_is_virtual', true === $this->get_virtual(), $this );
    }
    ...
    public function is_sold_individually() {
        return apply_filters( 'edgt_lms_course_is_sold_individually', true === $this->get_sold_individually(), $this );
    }
    ...
    public function is_visible() {
        ...
        if ( 'yes' === get_option( 'edgt_lms_course_hide_out_of_stock_items' ) && ! $this->is_in_stock() ) {
            $visible = false;
        }

        return apply_filters( 'edgt_lms_course_is_visible', $visible, $this->get_id() );
    }
    ...
    public function is_purchasable() {
        return apply_filters( 'edgt_lms_course_is_purchasable', $this->exists(), $this );
    }
    ...
    public function is_on_sale( $context = 'view' ) {
        ...
        return 'view' === $context ? apply_filters( 'edgt_lms_course_is_on_sale', $on_sale, $this ) : $on_sale;
    }
    ...
    public function is_in_stock() {
        return apply_filters( 'edgt_lms_course_is_in_stock', 'instock' === $this->get_stock_status(), $this );
    }
    ...
    */
    ...
    public function get_price_html( $deprecated = '' ) {
        if ( '' === $this->get_price() ) {
            return apply_filters( 'edgt_lms_course_empty_price_html', '', $this );
        }
        ...
        return apply_filters( 'edgt_lms_course_get_price_html', $price, $this );
    }
    ...
    public function add_to_cart_url() {
        return apply_filters( 'edgt_stock_photography_add_to_cart_url', $this->get_permalink(), $this );
    }
    ...
    public function single_add_to_cart_text() {
        return apply_filters( 'edgt_stock_photography_single_add_to_cart_text', __( 'Add to cart', 'edge-lms' ), $this );
    }
    ...
    public function add_to_cart_text() {
        return apply_filters( 'edgt_stock_photography_product_add_to_cart_text', __( 'Read more', 'edge-lms' ), $this );
    }
    ...
    public function get_file( $download_id = '' ) {
        ...
        return apply_filters( 'edgt_lms_course_product_file', $file, $this, $download_id );
    }
    ...
    public function get_file_download_path( $download_id ) {
        ...
        return apply_filters( 'edgt_lms_course_product_file_download_path', $file_path, $this, $download_id );
    }
    ...
    public function get_price_suffix( $price = '', $qty = 1 ) {
        ...
        if ( ( $suffix = get_option( 'edgt_lms_course_price_display_suffix' ) ) && wc_tax_enabled() && 'taxable' === $this->get_tax_status() ) {
            if ( '' === $price ) {
        ...
        return apply_filters( 'edgt_lms_course_get_price_suffix', $html, $this, $price, $qty );
    }
    ...
    public function get_availability() {
        return apply_filters( 'edgt_lms_course_get_availability', array(
    ...
    protected function get_availability_text() {
        if ( ! $this->is_in_stock() ) {
            $availability = __( 'Out of stock', 'edge-lms' );
        } elseif ( $this->managing_stock() && $this->is_on_backorder( 1 ) ) {
            $availability = __( 'Available on backorder', 'edge-lms' );
        } elseif ( $this->managing_stock() ) {
            $availability = wc_format_stock_for_display( $this );
        } else {
            $availability = '';
        }
        return apply_filters( 'edgt_lms_course_get_availability_text', $availability, $this );
    }
    ...
    protected function get_availability_class() {
        ...
        return apply_filters( 'edgt_lms_course_get_availability_class', $class, $this );
    }
