...
class EducatorEdgeCourseCategoriesWidget extends EducatorEdgeWidget {
    public function __construct() {
        parent::__construct(
            'edgt_course_categories_widget',
            esc_html__('Edge Course Categories Widget', 'edge-lms'),
            array( 'description' => esc_html__( 'Display list of your course categories', 'edge-lms'))
        );
	...
	protected function setParams() {
		$this->params = array(
            ...
            array(
                ...
                'title' => esc_html__( 'Number of Categories', 'edge-lms' )
            ),
            array(
                'type'  => 'textfield',
                'name'  => 'category',
                'title'       => esc_html__( 'Parent Category', 'edge-lms' ),
                'description' => esc_html__( 'Leave empty for all or fill in parent category slug', 'edge-lms' )
            ),
			array(
                'type'    => 'dropdown',
                'name'    => 'order_by',
                ...
                'options' => array(
                    'name'  => esc_html__('Name', 'edge-lms'),
                    'slug'  => esc_html__('Slug', 'edge-lms'),
                    'id'    => esc_html__('ID', 'edge-lms'),
                )
            ),
			array(
                'type'    => 'dropdown',
    ...
    public function widget($args, $instance) {
        ...
        echo '<div class="widget edgt-course-categories-widget">';
		    if ( ! empty( $instance['widget_title'] ) ) {
			    echo wp_kses_post( $args['before_title'] ) . esc_html( $instance['widget_title'] ) . wp_kses_post( $args['after_title'] );
		    }

		    if(!empty($terms)) {
		        echo '<ul class="edgt-course-categories-list">';
		        foreach ($terms as $term) {
					echo '<li>';
					echo '<' . $title_tag . ' class="edgt-course-categories-list-title">';
					if(edgt_lms_theme_installed()) {
						...
						if ($icon_pack != '') {
							...
							$icon = get_term_meta($term->term_id, 'course_category_' . $iconPackName, true);
							if ($icon != '') {
