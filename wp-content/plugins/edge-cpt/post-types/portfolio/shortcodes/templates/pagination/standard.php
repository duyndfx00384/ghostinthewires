<?php if($query_results->max_num_pages > 1) { ?>
	...
	<?php
		...
		if($pages > 1){ ?>
			<div class="edgt-pl-standard-pagination">
				<ul>
					<li class="edgt-pl-pag-prev">
					...
					<?php for ($i=1; $i <= $pages; $i++) { ?>
						<?php
							$active_class = '';
							if($paged == $i) {
								$active_class = 'edgt-pl-pag-active';
							}
						?>
						<li class="edgt-pl-pag-number <?php echo esc_attr($active_class); ?>">
							<a href="#" data-paged="<?php echo esc_attr($i); ?>"><?php echo esc_html($i); ?></a>
						</li>
					<?php } ?>
					<li class="edgt-pl-pag-next">
