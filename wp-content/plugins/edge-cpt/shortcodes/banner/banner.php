...
class Banner implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Image Overlay Color', 'edge-cpt' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'hover_behavior',
							'heading'     => esc_html__( 'Hover Behavior', 'edge-cpt' ),
							'value'       => array(
								esc_html__( 'Visible on Hover', 'edge-cpt' )   => 'edgt-visible-on-hover',
								esc_html__( 'Visible on Default', 'edge-cpt' ) => 'edgt-visible-on-default',
								esc_html__( 'Disabled', 'edge-cpt' )           => 'edgt-disabled'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Position', 'edge-cpt' ),
							'value'       => array(
								esc_html__( 'Default', 'edge-cpt' )  => 'default',
								esc_html__( 'Centered', 'edge-cpt' ) => 'centered'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Info Content Padding', 'edge-cpt' ),
							'description' => esc_html__( 'Please insert padding in format top right bottom left', 'edge-cpt' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'subtitle',
						...
						array(
							...
							'heading'     => esc_html__( 'Subtitle Tag', 'edge-cpt' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Title Top Margin (px)', 'edge-cpt' ),
							'dependency' => array( 'element' => 'title', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'    => esc_html__( 'Link Text Color', 'edge-cpt' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'link_top_margin',
							'heading'    => esc_html__( 'Link Text Top Margin (px)', 'edge-cpt' ),
							'dependency' => array( 'element' => 'link', 'not_empty' => true )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		$args   = array(
			...
			'image'                => '',
			'overlay_color'        => '',
			'hover_behavior'       => 'edgt-visible-on-hover',
		...
		$html = edgt_core_get_shortcode_module_template_part( 'templates/banner', 'banner', '', $params );
	...
	private function getHolderClasses( $params, $args ) {
		...
		$holderClasses[] = ! empty( $params['info_position'] ) ? 'edgt-banner-info-' . $params['info_position'] : 'edgt-banner-info-' . $args['info_position'];
			...
			if ( ! empty( $title_light_words ) ) {
				foreach ( $light_words as $value ) {
					if ( ! empty( $split_title[ $value - 1 ] ) ) {
						$split_title[ $value - 1 ] = '<span class="edgt-banner-title-light">' . $split_title[ $value - 1 ] . '</span>';
					}
				}
			}
	...
	private function getTitleStyles( $params ) {
		...
		if ( ! empty( $params['title_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . educator_edge_filter_px( $params['title_top_margin'] ) . 'px';
		}
	...
	private function getLinkStyles( $params ) {
		...
		if ( ! empty( $params['link_top_margin'] ) ) {
			$styles[] = 'margin-top: ' . educator_edge_filter_px( $params['link_top_margin'] ) . 'px';
		}
