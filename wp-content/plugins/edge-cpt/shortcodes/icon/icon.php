...
class Icon implements Lib\ShortcodeInterface {
		    ...
		    vc_map(
		    	array(
				    ...
				    'params'                    => array_merge(
					    ...
					    \EducatorEdgeIconCollections::get_instance()->getVCParamsArray(),
					    array(
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'size',
							    ...
							    'value'      => array(
								    ...
								    esc_html__( 'Huge', 'edge-cpt' )   => 'edgt-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
						    ...
						    array(
							    ...
							    'dependency'  => array( 'element' => 'type', 'value' => array( 'edgt-square' ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'shape_size',
							    'heading'    => esc_html__( 'Shape Size (px)', 'edge-cpt' ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'edgt-circle', 'edgt-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
	...
	public function render( $atts, $content = null ) {
		...
		$html = edgt_core_get_shortcode_module_template_part( 'templates/icon', 'icon', '', $params );
		
		return $html;
	}
	
	private function generateIconHolderClasses( $params ) {
		$holderClasses = array( 'edgt-icon-shortcode', $params['type'] );
		
		$holderClasses[] = ! empty( $params['custom_class'] ) ? esc_attr( $params['custom_class'] ) : '';
		$holderClasses[] = $params['icon_animation'] == 'yes' ? 'edgt-icon-animation' : '';
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'edgt-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'edgt-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . educator_edge_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'edgt-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . educator_edge_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . educator_edge_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . educator_edge_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . educator_edge_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'edgt-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . educator_edge_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
	...
	private function generateIconHolderData( $params ) {
		...
		if ( isset( $params['type'] ) && $params['type'] !== 'edgt-normal' ) {
