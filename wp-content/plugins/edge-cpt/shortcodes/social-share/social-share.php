...
class SocialShare implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						array(
							...
							'value'      => array(
								esc_html__( 'List', 'edge-cpt' )     => 'list',
								esc_html__( 'Dropdown', 'edge-cpt' ) => 'dropdown'
							)
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'icon_type',
							'heading'    => esc_html__( 'Icons Type', 'edge-cpt' ),
							'value'      => array(
								esc_html__( 'Font Awesome', 'edge-cpt' ) => 'font-awesome',
								esc_html__( 'Font Elegant', 'edge-cpt' ) => 'font-elegant'
							)
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'title',
							'heading'    => esc_html__( 'Social Share Title', 'edge-cpt' ),
	...
	public function render( $atts, $content = null ) {
		...
		$params['enable_social_share'] = ( educator_edge_options()->getOptionValue( 'enable_social_share' ) == 'yes' ) ? true : false;
		
		//Is social share enabled for post type
		$post_type         = get_post_type();
		$params['enabled'] = ( educator_edge_options()->getOptionValue( 'enable_social_share_on_' . $post_type ) == 'yes' ) ? true : false;
		...
		if ( $params['enable_social_share'] ) {
			if ( $params['enabled'] ) {
				$html .= edgt_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'social-share', '', $params );
			}
		}
	...
	private function getSocialNetworksParams( $params ) {
		...
		foreach ( $this->socialNetworks as $net ) {
			...
			if ( educator_edge_options()->getOptionValue( 'enable_' . $net . '_share' ) == 'yes' ) {
				...
				$params['custom_icon'] = ( educator_edge_options()->getOptionValue( $net . '_icon' ) ) ? educator_edge_options()->getOptionValue( $net . '_icon' ) : '';
				
				$html = edgt_core_get_shortcode_module_template_part( 'templates/parts/network', 'social-share', '', $params );
			}
    ...
    private function getSocialNetworkShareLink($net, $image) {
        switch ($net) {
            ...
            case 'twitter':
                ...
                $twitter_via = (educator_edge_options()->getOptionValue('twitter_via') !== '') ? ' via ' . educator_edge_options()->getOptionValue('twitter_via') . ' ' : '';
                if (wp_is_mobile()) {
                    $link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(educator_edge_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                } else {
                    $link = 'window.open(\'http://twitter.com/home?status=' . urlencode(educator_edge_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
                }
                break;
            case 'google_plus':
            ...
            case 'tumblr':
                ...
                break;
            case 'pinterest':
                $link = 'popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()) . '&amp;description=' . educator_edge_addslashes(get_the_title()) . '&amp;media=' . urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;';
                break;
            case 'vk':
	...
	private function getSocialNetworkIcon( $net, $type ) {
		switch ( $net ) {
			...
			case 'twitter':
				...
				break;
			case 'google_plus':
				$icon = ( $type == 'font-awesome' ) ? 'social_googleplus' : 'fa fa-google-plus';
				break;
			case 'linkedin':
