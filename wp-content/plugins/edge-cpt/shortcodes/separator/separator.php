...
class Separator implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                  => array(
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'width',
							'heading'    => esc_html__( 'Width (px or %)', 'edge-cpt' ),
							'dependency' => array( 'element' => 'type', 'value' => array( 'normal' ) )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'    => esc_html__( 'Top Margin (px or %)', 'edge-cpt' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'bottom_margin',
							'heading'    => esc_html__( 'Bottom Margin (px or %)', 'edge-cpt' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$html = edgt_core_get_shortcode_module_template_part( 'templates/separator-template', 'separator', '', $params );
	...
	private function getHolderClasses( $params ) {
		...
		$holderClasses[] = ! empty( $params['position'] ) ? 'edgt-separator-' . $params['position'] : '';
		$holderClasses[] = ! empty( $params['type'] ) ? 'edgt-separator-' . $params['type'] : '';
	...
	private function getHolderStyles( $params ) {
		...
		if ( $params['width'] !== '' ) {
			if ( educator_edge_string_ends_with( $params['width'], '%' ) || educator_edge_string_ends_with( $params['width'], 'px' ) ) {
				$styles[] = 'width: ' . $params['width'];
			} else {
				$styles[] = 'width: ' . educator_edge_filter_px( $params['width'] ) . 'px';
			}
		}
		
		if ( $params['thickness'] !== '' ) {
			$styles[] = 'border-bottom-width: ' . educator_edge_filter_px( $params['thickness'] ) . 'px';
		}
		
		if ( $params['top_margin'] !== '' ) {
			if ( educator_edge_string_ends_with( $params['top_margin'], '%' ) || educator_edge_string_ends_with( $params['top_margin'], 'px' ) ) {
				$styles[] = 'margin-top: ' . $params['top_margin'];
			} else {
				$styles[] = 'margin-top: ' . educator_edge_filter_px( $params['top_margin'] ) . 'px';
			}
		}
		
		if ( $params['bottom_margin'] !== '' ) {
			if ( educator_edge_string_ends_with( $params['bottom_margin'], '%' ) || educator_edge_string_ends_with( $params['bottom_margin'], 'px' ) ) {
				$styles[] = 'margin-bottom: ' . $params['bottom_margin'];
			} else {
				$styles[] = 'margin-bottom: ' . educator_edge_filter_px( $params['bottom_margin'] ) . 'px';
			}
		}
