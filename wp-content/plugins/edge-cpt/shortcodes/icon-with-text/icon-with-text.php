...
class IconWithText implements Lib\ShortcodeInterface {
					...
					'params'                    => array_merge(
						array(
							...
							array(
								...
								'value'       => array(
									esc_html__( 'Icon Left From Text', 'edge-cpt' )  => 'icon-left',
									esc_html__( 'Icon Left From Title', 'edge-cpt' ) => 'icon-left-from-title',
									esc_html__( 'Icon Top', 'edge-cpt' )             => 'icon-top'
								),
								'save_always' => true
							)
						),
						...
						array(
							...
							array(
								...
								'dependency' => array(
									'element' => 'icon_type',
									'value'   => array( 'edgt-square', 'edgt-circle' )
								),
								'group'      => esc_html__( 'Icon Settings', 'edge-cpt' )
							),
							array(
							...
							array(
								...
								'description' => esc_html__( 'Set link around icon and title', 'edge-cpt' )
							),
							array(
								'type'       => 'dropdown',
								'param_name' => 'target',
							...
							array(
								...
								'heading'     => esc_html__( 'Text Padding (px)', 'edge-cpt' ),
								'description' => esc_html__( 'Set left or top padding dependence of type for your text holder. Default value is 13 for left type and 25 for top icon with text type', 'edge-cpt' ),
	...
	public function render( $atts, $content = null ) {
		...
		return edgt_core_get_shortcode_module_template_part( 'templates/iwt', 'icon-with-text', $params['type'], $params );
	}
	...
	private function getHolderClasses( $params ) {
		$holderClasses = array( 'edgt-iwt', 'clearfix' );
		
		$holderClasses[] = ! empty( $params['custom_class'] ) ? esc_attr( $params['custom_class'] ) : '';
		$holderClasses[] = ! empty( $params['type'] ) ? 'edgt-iwt-' . $params['type'] : '';
		$holderClasses[] = ! empty( $params['icon_size'] ) ? 'edgt-iwt-' . str_replace( 'edgt-', '', $params['icon_size'] ) : '';
	...
	private function getContentStyles( $params ) {
		...
		if ( $params['text_padding'] !== '' && $params['type'] === 'icon-left' ) {
			$styles[] = 'padding-left: ' . educator_edge_filter_px( $params['text_padding'] ) . 'px';
		}
		
		if ( $params['text_padding'] !== '' && $params['type'] === 'icon-top' ) {
			$styles[] = 'padding-top: ' . educator_edge_filter_px( $params['text_padding'] ) . 'px';
		}
	...
	private function getTextStyles( $params ) {
		...
		if ( $params['text_top_margin'] !== '' ) {
			$styles[] = 'margin-top: ' . educator_edge_filter_px( $params['text_top_margin'] ) . 'px';
		}
