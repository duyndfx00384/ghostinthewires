...
class IconListItem implements Lib\ShortcodeInterface {
					...
					'params'   => array_merge(
						array(
							...
							array(
								...
								'heading'     => esc_html__( 'Icon List Item Bottom Margin (px)', 'edge-cpt' ),
								'description' => esc_html__( 'Set bottom margin for your Icon List Item element. Default value is 8', 'edge-cpt' )
							)
						),
						...
						array(
							...
							array(
								...
								'heading'     => esc_html__( 'Title Left Padding (px)', 'edge-cpt' ),
								'description' => esc_html__( 'Set left padding for your text element to adjust space between icon and text. Default value is 13', 'edge-cpt' ),
								'dependency'  => Array( 'element' => 'title', 'not_empty' => true )
							)
						)
					)
	...
	public function render( $atts, $content = null ) {
		...
		$html = edgt_core_get_shortcode_module_template_part( 'templates/icon-list-item-template', 'icon-list-item', '', $params );
	...
	private function getHolderStyles( $params ) {
		...
		if ( $params['item_margin'] !== '' ) {
			$styles[] = 'margin-bottom: ' . educator_edge_filter_px( $params['item_margin'] ) . 'px';
		}
	...
	private function getIconStyles( $params ) {
		...
		if ( ! empty( $params['icon_size'] ) ) {
			$styles[] = 'font-size: ' . educator_edge_filter_px( $params['icon_size'] ) . 'px';
		}
	...
	private function getTitleStyles( $params ) {
		...
		if ( ! empty( $params['title_size'] ) ) {
			$styles[] = 'font-size: ' . educator_edge_filter_px( $params['title_size'] ) . 'px';
		}
		
		if ( $params['title_padding'] !== '' ) {
			$styles[] = 'padding-left: ' . educator_edge_filter_px( $params['title_padding'] ) . 'px';
		}
