...
class EducatorEdgeFramework {
    ...
    private function __construct() {
        $this->edgtOptions = EducatorEdgeOptions::get_instance();
        $this->edgtMetaBoxes = EducatorEdgeMetaBoxes::get_instance();
    }
    ...
    public function setSkin(EducatorEdgeSkinAbstract $skinObject) {
        $this->skin = $skinObject;
    }
}

/**
 * Class EducatorEdgeSkinManager
 *
    ...
    private function setSkin($skinName = 'edge') {
        if ($skinName !== '') {
            if (file_exists(get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php')) {
                ...
                $skinName = ucfirst($skinName) . esc_html__('Skin', 'educator');
...
/**
 * Class EducatorEdgeSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class EducatorEdgeSkinAbstract
{
    /**
     * @var string
     */
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if ($template !== '') {
            include(educator_edge_find_template_path('framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
        <script>
            window.edgtSCIcon = '<?php echo educator_edge_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
...
class EducatorEdgeOptions
{
    ...
    public function getOptionValue($key) {
        ...
        if (array_key_exists($key, $educator_edge_options)) {
            return $educator_edge_options[$key];
...
/*
   Class: EducatorEdgeAdminPage
   A class that initializes Edge Admin Page
*/

class EducatorEdgeAdminPage implements iEducatorEdgeLayoutNode
{
    ...
    function __construct($slug = "", $title = "", $icon = "") {
        $this->layout = array();
        $this->factory = new EducatorEdgeFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iEducatorEdgeRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: EducatorEdgeMetaBoxes
...
/*
   Class: EducatorEdgeMetaBox
   A class that initializes Edge Meta Box
*/

class EducatorEdgeMetaBox implements iEducatorEdgeLayoutNode
{
    ...
    public function setTitle($label) {
        ...
        return $educator_edge_Framework->getSkin()->getSkinLabel() . ' ' . $label;
    }
}

if (!function_exists('educator_edge_init_framework_variable')) {
    function educator_edge_init_framework_variable() {
        global $educator_edge_Framework;

        $educator_edge_Framework = EducatorEdgeFramework::get_instance();
        $edgtSkinManager = new EducatorEdgeSkinManager();
        $educator_edge_Framework->setSkin($edgtSkinManager->getSkin());
    }

    add_action('educator_edge_before_options_map', 'educator_edge_init_framework_variable');
}
?>