...
class EducatorEdgeSocialIconWidget extends EducatorEdgeWidget {
    public function __construct() {
        parent::__construct(
            'edgt_social_icon_widget',
            esc_html__('Edge Social Icon Widget', 'educator'),
            array( 'description' => esc_html__( 'Add social network icons to widget areas', 'educator'))
        );
	...
	protected function setParams() {
		$this->params = array_merge(
			educator_edge_icon_collections()->getSocialIconWidgetParamsArray(),
			array(
				array(
					'type'  => 'textfield',
					'name'  => 'link',
    ...
    public function widget($args, $instance) {
	    ...
	    if ( ! empty( $instance['icon_size'] ) ) {
		    $icon_styles[] = 'font-size: ' . educator_edge_filter_px( $instance['icon_size'] ) . 'px';
	    }
	    ...
	    if ( ! empty( $instance['icon_pack'] ) ) {
		    $icon_class   = array( 'edgt-social-icon-widget' );
        ...
        <a class="edgt-social-icon-widget-holder edgt-icon-has-hover" <?php echo educator_edge_get_inline_attr($hover_color, 'data-hover-color'); ?> <?php educator_edge_inline_style($icon_styles) ?> href="<?php echo esc_url($link); ?>" target="<?php echo esc_attr($target); ?>">
