...
if (!class_exists('EducatorEdgeStickyNavigationWalker')) {
	class EducatorEdgeStickyNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if($item->sidebar != "" && $depth > 0){
                $sidebar_class = 'uses-custom-sidebar';
            }

			// build html
			$output .= $indent . '<li id="sticky-nav-menu-item-'. $item->ID . '" class="' . $class_names . ' '. $active . $sub . $menu_type . $wide_menu_position . $sidebar_class .'">';
			...
			if($item->hide == ""){
                ...
                if($item->featured_icon !== ""){
                    $featured_icon .= '<i class="edgt-menu-featured-icon fa '.$item->featured_icon .'"></i>';
                }
				...
				if($args->has_children && $depth == 0) {
					$item_output .= '<i class="edgt-menu-arrow fa fa-angle-down"></i>';
				}
