 ...
 */
if ( ! class_exists( 'EducatorEdgeSidebar' ) ) {
	class EducatorEdgeSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'educator' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'edgt_sidebar', EDGE_ROOT . '/framework/admin/assets/js/edgt-sidebar.js' );
			wp_enqueue_style( 'edgt_sidebar', EDGE_ROOT . '/framework/admin/assets/css/edgt-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_html__('Enter Name of the new Widget Area', 'educator') . "' name='edgt-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_html__('Add Widget Area', 'educator') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'educator_edge_custom_widget_args', $args );
