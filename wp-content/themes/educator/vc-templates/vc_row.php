...
$edgt_row_wrapper_start = $edgt_row_wrapper_end = '';
...
$grid_row_class = $grid_row_data = $edgt_vc_row_style = $edgt_grid_row_style = array();

if ( $row_content_width !== 'grid' ) {
	if (!empty($disable_background_image)){
		$css_classes[] = 'edgt-disabled-bg-image-bellow-' . esc_attr($disable_background_image);
	}
	
	if ( ! empty( $simple_background_color ) ) {
		$edgt_vc_row_style[] = 'background-color:' . esc_attr( $simple_background_color );
	}
	
	if ( ! empty( $simple_background_image ) ) {
		$image_src            = wp_get_attachment_image_src($simple_background_image, 'full');
		$edgt_vc_row_style[] = 'background-image: url(' . esc_url( $image_src[0] ) . ')';
	}
	
	if (!empty($parallax_background_image)) {
		$image_src = wp_get_attachment_image_src($parallax_background_image, 'full');
		
		$css_classes[] = 'edgt-parallax-row-holder';
	...
	if (!empty($content_text_aligment)){
		$css_classes[] = 'edgt-content-aligment-' . esc_attr($content_text_aligment);
	}
	
} else {
	if (!empty($disable_background_image)){
		$grid_row_class[] = 'edgt-disabled-bg-image-bellow-' . esc_attr($disable_background_image);
	}
	
	if ( ! empty( $simple_background_color ) ) {
		$edgt_grid_row_style[] = 'background-color:' . esc_attr( $simple_background_color );
	}
	
	if ( ! empty( $simple_background_image ) ) {
		$image_src            = wp_get_attachment_image_src($simple_background_image, 'full');
		$edgt_grid_row_style[] = 'background-image: url(' . esc_url( $image_src[0] ) . ')';
	}
	
	if (!empty($parallax_background_image)) {
		$image_src = wp_get_attachment_image_src($parallax_background_image, 'full');
		
		$grid_row_class[] = 'edgt-parallax-row-holder';
	...
	if (!empty($content_text_aligment)){
		$grid_row_class[] = 'edgt-content-aligment-' . esc_attr($content_text_aligment);
	}
}
...
$edgt_vc_row_styles = '';
if( ! empty( $edgt_vc_row_style ) ) {
	$edgt_vc_row_styles = implode( ';', $edgt_vc_row_style );
}

$edgt_grid_row_styles = '';
if( ! empty( $edgt_grid_row_style ) ) {
	$edgt_grid_row_styles = implode( ';', $edgt_grid_row_style );
}

if($row_content_width === 'grid'){
	$edgt_row_wrapper_start .= '<div class="edgt-row-grid-section-wrapper '.esc_attr($grid_row_classes).'" '.esc_attr($grid_row_datas).' ' . educator_edge_get_inline_style( $edgt_grid_row_styles ) . '><div class="edgt-row-grid-section">';
	$edgt_row_wrapper_end .= '</div></div>';
}
...
$output .= $edgt_row_wrapper_start;
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . educator_edge_get_inline_style( $edgt_vc_row_styles ) . '>';
$output .= wpb_js_remove_wpautop( $content );
$output .= '</div>';
$output .= $edgt_row_wrapper_end;
$output .= $after_output;
